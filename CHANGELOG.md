...
### 新增
- ✨ 分屏视图模式 - 新增编辑器模式和分屏模式切换，左侧编辑右侧实时预览树形结构
- ✨ 树形 JSON 展示 - 分屏模式下右侧展示可折叠的树形 JSON 结构，支持展开/折叠全部节点
- ✨ 可拖动分割线 - 分屏模式下支持拖动调整左右面板宽度
- ✨ 字符串折叠功能 - 支持折叠/展开长字符串值

### 优化
- 💄 优化编辑器界面布局 - 调整 JSON 显示区域高度计算，防止内容超出可视区域
- 💄 优化树形视图底部间距 - 减少 tree-view 下方空白区域
- 💄 优化窗口调整大小体验 - 修复调整窗口大小时程序卡死问题
- 💄 优化转义字符显示 - 修复转义状态下 `\n` 换行符无法正确显示的问题
