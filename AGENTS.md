...
- `src/utils/windowController.ts` — Tauri window detection and minimize/maximize/close operations
- `src/utils/splitResizer.ts` — Split-panel drag lifecycle and listener cleanup
...
**State management**: Persisted UI preferences (theme, view mode, update setting) live in `localStorage`; the current JSON source has one in-memory canonical value mirrored to both legacy textareas. Backend tray menu state uses `Arc<Mutex>` for thread safety and syncs through Tauri events.
