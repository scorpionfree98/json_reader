...
- 分屏支持“解析字符串类型json”和“富内容”两个复选框：
  - “解析字符串类型json”只在最外层 JSON 值是字符串时，再解析一次字符串内部的 JSON。
...
- ~~开始 Task 5 前，固定窗口尺寸和测试数据，保存编辑器/分屏模式在明暗主题下的四组基线截图。~~ 已保存于 `docs/visual-baselines/6b30f6d/`，视口为 1100 × 760。
...
- 断言关闭“解析字符串类型json”后，外层字符串保持字符串，不递归修改对象字段。
...
- 主界面与分屏统一使用“解析字符串类型json”，tooltip 明确只解析最外层字符串一次。
