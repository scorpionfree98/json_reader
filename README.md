...
- 🔎 **结果视图搜索** - 树形和高亮结果均支持大小写敏感、正则表达式和匹配导航
- 🧭 **统一 JSON 工作台** - 使用唯一输入源，在树形与高亮结果间切换，支持编辑、双栏和结果布局
...
- 🧩 **解析字符串类型json** - 可解析最外层 JSON 字符串中的一层 JSON 内容
...
4. **切换布局与结果**
   - 布局支持仅编辑、双栏和仅结果，桌面默认双栏
   - 结果支持树形与高亮两种视图，切换不会修改或重新解析输入
   - 拖动中间分割线可调整两侧宽度；窄窗口使用编辑/结果按钮切换主面板
